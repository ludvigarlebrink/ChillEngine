...
void Font::LoadTTF(const std::string& filename, int32 fontSize)
{
    ...
    for (uchar c = 0u; c < 128u; ++c)
    {
        ...
        {
            m_characters[c].size = Vector2i(face->glyph->bitmap.width, face->glyph->bitmap.rows);
            m_characters[c].bearing = Vector2i(face->glyph->bitmap_left, face->glyph->bitmap_top);
    ...
    m_pTexture->Create(textureSize, textureSize, Texture::Format::R, nullptr);
    ...
    for (uchar c = 0u; c < 128u; ++c)
    {
        if (!FT_Load_Char(face, c, FT_LOAD_RENDER))
        {
            ...
            m_pTexture->CreateSubTexture(x, y, face->glyph->bitmap.width, face->glyph->bitmap.rows, face->glyph->bitmap.buffer);
            m_characters[c].texturePosition = Vector2f(static_cast<f32>(x) / static_cast<f32>(textureSize), static_cast<f32>(y) / static_cast<f32>(textureSize));
            m_characters[c].textureSize = Vector2f(static_cast<f32>(m_characters[c].size.x) / static_cast<f32>(textureSize), static_cast<f32>(m_characters[c].size.y) / static_cast<f32>(textureSize));
        }
    }
